/*
  From https://github.com/Lotiq/DPM8600/tree/b3ccdd4997f6b61f72395278b48b375f6f59b3e5
  ...
  begin(serial, maxRetry) - Initialization. Returns [Int] 1  on success, or negative error number on error.
    ...
    * maxRetry - [int8_t] - maximum number of retries upon failing to communicate with the power converter before
  generating an error. DEFAULT = 3
  ...
  write(cmd, value) - writing command. CMD is a character, upper or lower case that designates the command to be used.
  Value is the value that is being set.
  ...
  Errors are returned as NEGATIVE numbers for ALL functions. E.g. read('v') can return -10, indicating an error. This
  way you can check if the function succeeds for any of them. -1 - error on begining DPM8600. -10 - error on voltage
  reading -11 - error on current reading. -12 - error on power reading. -13 - error on CC/CV reading. -14 - error on max
  current reading. -15 - error on temperature reading. -16 - value outside boundaries -20 - wrong value sent to power
  writing function -21 - error setting current -22 - error setting power on/off. -23 - error setting voltage -24 - error
  setting current and voltage -25 - wrong writing command
*/
...
class DPM8600 {
   public:
    ...
    int begin(SoftwareSerial *serial, int8_t maxRetry = 5);
    int power(bool on);
